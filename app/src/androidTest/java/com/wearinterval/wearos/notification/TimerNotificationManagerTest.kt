...
class TimerNotificationManagerTest {
  ...
  fun showTimerAlertWithAllNotificationsEnabled() {
    ...
    val alertNotification =
      activeNotifications.find { it.id == TimerNotificationManager.ALERT_NOTIFICATION_ID }

    assertThat(alertNotification).isNotNull()
    assertThat(alertNotification?.notification?.channelId)
      .isEqualTo(TimerNotificationManager.ALERT_CHANNEL_ID)
  }
  ...
  fun showTimerAlarmWithContinuousAlert() {
    ...
    assertThat(
        alarmNotification?.notification?.flags?.and(android.app.Notification.FLAG_ONGOING_EVENT)
      )
      .isNotEqualTo(0)
  ...
  fun updateTimerNotificationChangesActiveNotification() {
    ...
    val timerNotification =
      activeNotifications.find { it.id == TimerNotificationManager.TIMER_NOTIFICATION_ID }
  ...
  fun dismissAlertRemovesNotification() {
    ...
    var alertExists =
      activeNotifications.any { it.id == TimerNotificationManager.ALERT_NOTIFICATION_ID }
