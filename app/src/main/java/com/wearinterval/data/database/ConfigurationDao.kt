...
interface ConfigurationDao {
    ...
    @Query("SELECT COUNT(*) FROM timer_configurations")
    suspend fun getConfigurationCount(): Int
    
    @Query("DELETE FROM timer_configurations WHERE id NOT IN (SELECT id FROM timer_configurations ORDER BY lastUsed DESC LIMIT :keepCount)")
