...
data class TimerConfiguration(
    ...
    fun displayString(): String {
        ...
        val restText = if (restDuration > Constants.TimerLimits.MIN_REST_DURATION) " + ${TimeUtils.formatDuration(restDuration)}" else ""
    ...
    companion object {
        ...
        fun validate(laps: Int, workDuration: Duration, restDuration: Duration): TimerConfiguration {
            ...
            val validWorkDuration = workDuration.coerceIn(Constants.TimerLimits.MIN_WORK_DURATION, Constants.TimerLimits.MAX_WORK_DURATION)
            val validRestDuration = restDuration.coerceIn(Constants.TimerLimits.MIN_REST_DURATION, Constants.TimerLimits.MAX_REST_DURATION)
        ...
        val COMMON_PRESETS = listOf(
            ...
            TimerConfiguration(laps = 1, workDuration = 60.seconds, restDuration = Constants.TimerLimits.MIN_REST_DURATION),
            TimerConfiguration(laps = 1, workDuration = 2.minutes, restDuration = Constants.TimerLimits.MIN_REST_DURATION),

            // Interval training
            TimerConfiguration(laps = 5, workDuration = 45.seconds, restDuration = Constants.TestValues.COMMON_REST_DURATION),
            ...
            // Infinite workout
            TimerConfiguration(laps = Constants.TimerLimits.INFINITE_LAPS, workDuration = 25.seconds, restDuration = 5.seconds),
        )
    }
}
