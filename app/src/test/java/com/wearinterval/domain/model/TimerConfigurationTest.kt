...
class TimerConfigurationTest {
  ...
  fun `COMMON_PRESETS are all valid`() {
    TimerConfiguration.COMMON_PRESETS.forEach { preset -> assertThat(preset.isValid()).isTrue() }
  }
  ...
  fun `COMMON_PRESETS include expected configurations`() {
    ...
    val hasThirtySecond =
      presets.any { it.laps == 1 && it.workDuration == 30.seconds && it.restDuration == 0.seconds }
    ...
    val hasTabata =
      presets.any { it.laps == 8 && it.workDuration == 25.seconds && it.restDuration == 5.seconds }
