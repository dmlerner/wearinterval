...
class WearOsRepositoryTest {
    ...
    fun setup() {
        ...
        every { mockConfigurationRepository.recentConfigurations } returns MutableStateFlow(recentConfigs)
    ...
    fun `getComplicationData ShortText handles infinite laps`() = runTest {
        // Given
        val infiniteConfig = TimerConfiguration(laps = 999, workDuration = 30.seconds, restDuration = 0.seconds)
        val runningState = TimerState(
            phase = TimerPhase.Running,
    ...
    fun `formatTime handles minutes and seconds correctly`() = runTest {
        ...
        val config = TimerConfiguration(laps = 1, workDuration = 2.minutes + 30.seconds, restDuration = 0.seconds)
        val longDurationState = TimerState(
            phase = TimerPhase.Running,
        ...
        // When
        val data = repository.getComplicationData(ComplicationType.ShortText) as ComplicationData.ShortText
