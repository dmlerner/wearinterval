...
/**
 * Unit tests for theme-related constants and logic.
 * UI theme tests should be in androidTest directory.
 */
class ThemeTest {
    ...
    fun colorValues_areValid() {
        ...
        colorFormats.forEach { color ->
            assertThat(isValidColorHex(color)).isTrue()
        }
    }
    ...
    fun typography_scale_isConsistent() {
        ...
        sizes.zipWithNext().forEach { (larger, smaller) ->
            assertThat(larger).isAtLeast(smaller)
        }
    ...
    fun spacing_values_areConsistent() {
        ...
        spacingValues.forEach { spacing ->
            assertThat(spacing % 4f).isEqualTo(0f)
        }
    ...
    fun dark_theme_coverage() {
        ...
        onProperties.forEach { onProperty ->
            ...
            val hasBaseProperty = themeProperties.any {
                it.lowercase() == baseProperty
            }
    ...
    fun color_semantic_naming() {
        ...
        semanticColors.forEach { (colorName, description) ->
            ...
            val hasSpecificColorName = colorName.lowercase().let { name ->
                name.contains("red") || name.contains("blue") || name.contains("green") || name.contains("yellow")
            }
